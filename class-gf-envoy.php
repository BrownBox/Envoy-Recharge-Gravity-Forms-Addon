...
if (class_exists("GFForms")) {
    ...
    class GFEnvoy extends GFPaymentAddOn{
        ...
        protected $_path = "gf-add-on/envoyrecharge.php";
        ...
        public static function get_instance()
        {
            ...
            self::$_instance->form = parent::get_current_form();
        ...
        public function pre_init(){
            ...
            array_push($this->gateways,array(
                                    "label" => __('Select a gateway', 'gravityformsenvoy'),
                                    "value" => ""
                                ));
            ...
            if(sizeof($gatewaysObject->gateways) > 0){
                foreach ($gatewaysObject->gateways as $key => $gateway) {
                    array_push($this->gateways,array(
                                        "label" => __($gateway->name, 'gravityformsenvoy'),
                                        "value" => $gateway->_id
                                    ));
                }
            }
        ...
        { ?>
            ...
            <div class="about-text">Thank you for using EnvoyRecharge standard Add-On! The new version of the Gravity Forms EnvoyRecharge Standard Add-On makes easy  how you manage your reccuring payments.</div>
            <div class="changelog">
                <hr/>
                <div class="feature-section col two-col">
                    <div class="col-1">
                        <h3>Manage EnvoyRecharge Contextually</h3>
                        <p>EnvoyRecharge Feeds are now accessed via the EnvoyRecharge sub-menu within the Form Settings for the Form you would like to integrate EnvoyRecharge with.</p>
                    </div>
              ...
              <p><a href="http://envoyrecharge.com/" target="_blank">Visit us to know more</a>
               or <a href="admin.php?page=gf_settings&subview=EnvoyRecharge"> Go to Settings</a></p> 
        ...
        public function feed_settings_fields()
        {
            ...
            //$checkboxradio_field_settings = self::$_instance->ChexkboxRadioFields();
            ...
            $fields = array(
                array(
                    ...
                    "fields" => array(
                        array(
                            "label" => __('Name', 'gravityformsenvoy'),
                            "type" => "text",
                            "name" => "feedName",
                            "required" => true,
                            "tooltip" => __('Enter a feed name to uniquely identify this setup','gravityformsenvoy')
                        ),
                        
                        array(
                            "label" => __("Gateway",'gravityformsenvoy'),
                            "type" => "select",
                            "name" => "envoygateway_" . $form['id'],
                            "tooltip" => __('Choose a payment Gateway','gravityformsenvoy'),
                            "id" => "envoygatewayid",
                            "required" => true,
                        ...
                        array(
                            ...
                            "choices" => array(
                                array(
                                    "label" => __('subscription to envoy', 'gravityformsenvoy'),
                                    "value" => "subscription"
                                )
        ...
        protected function generalSettings()
        {
            ...
            array_push($fields, array(
                "label" => __('Amount', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Currency', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('First Name', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Last Name', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Email', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Address', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Address2', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('City', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('State', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Zip/Post code', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Country', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Start date', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('End date', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Reference for payment', 'gravityformsenvoy'),
                "type" => "select",
            ...
            array_push($fields, array(
                "label" => __('Reference for 3rd party integration', 'gravityformsenvoy'),
                "type" => "select",
                ...
                if($field["type"] !="creditcard"){
                    ...
                    if($field["type"]=='address' || ($field["type"]=='name' && $field["nameFormat"]!='simple')){
                        foreach ($field['inputs'] as $keyvalue => $inputvalue) {
                            ...
                            $field_settings['label'] = __($inputvalue['label'],'gravityformsenvoy');
                        ...
                        $field_settings['label'] = __($field['label'],'gravityformsenvoy');
                        array_push($default_settings, $field_settings);
                    }
        ...
        protected function productFields()
        {
            ...
            foreach ($fields as $key => $field) {
                if ($field['type'] == 'product' || $field['type'] == 'total') {
                    if($field['type'] == 'total') $check_total_exist = 1; //total exists.
                    $field_settings          = array();
                    $field_settings['value'] = $field['id'];
                    $field_settings['label'] = __($field['label'],'gravityformsenvoy');
                    array_push($default_settings, $field_settings);
            ...
            if($check_total_exist == 0){
                ...
                $field_settings['label'] = __('Total','gravityformsenvoy');
                array_push($default_settings, $field_settings);
            } 
            return $default_settings;
        }
        ...
        protected function ChexkboxRadioFields()
        {
            $form = $this->get_current_form();
            $fields           = $form['fields'];
            $default_settings = array();
            array_push($default_settings,array(
                                    "label" => __('Select a transaction type', 'gravityformsenvoy'),
                                    "value" => ""
                                ));
            foreach ($fields as $key => $field) {
...
?>
            ...
            return array(
                array(
                    ...
                    "fields" => array(
                        array(
                            "name" => "envoyapikey",
                            "tooltip" => __('Your EnvoyRecharge Key','gravityformsenvoy'),
                            "label" => __('API Key','gravityformsenvoy'),
                            "type" => "text",
                            "class" => "small"
                        )
                    )
        ...
        public function envoyValidateForm($validation_result)
        {
            ...
            if ($feed) {
                ...
                if(empty($fname_fid)) array_push($errors, __('Error: EnvoyRecharge firstname feed field is empty.','gravityformsenvoy'));
                else
                    $fname_idarray = array('firstname',$fname_fid);

                $lname_fid         = $envoyrechargemeta['envoylname_' . $form_id];
                if(empty($lname_fid)) array_push($errors, __('Error: EnvoyRecharge lastname feed field is empty.','gravityformsenvoy'));
                else
                    $lname_idarray = array('lastname',$lname_fid);

                $email_fid         = $envoyrechargemeta['envoyemail_' . $form_id];
                if(empty($email_fid)) array_push($errors, __('Error: EnvoyRecharge email feed field is empty.','gravityformsenvoy'));
                else
                    $email_idarray = array('email',$email_fid);

                $address_fid       = $envoyrechargemeta['envoyaddress_' . $form_id];
                if(empty($address_fid)) array_push($errors, __('Error: EnvoyRecharge address feed field is empty.','gravityformsenvoy'));
                else
                    $address_idarray = array('address_line1',$address_fid);

                $address2_fid      = $envoyrechargemeta['envoyaddress2_' . $form_id];
                if(!empty($address2_fid)) $address2_idarray = array('address_line2',$address2_fid);

                $city_fid          = $envoyrechargemeta['envoycity_' . $form_id];
                if(empty($city_fid)) array_push($errors, __('Error: EnvoyRecharge city feed field is empty.','gravityformsenvoy'));
                else
                    $city_idarray = array('address_city',$city_fid);

                $state_fid         = $envoyrechargemeta['envoystate_' . $form_id];
                if(empty($state_fid)) array_push($errors, __('Error: EnvoyRecharge state feed field is empty.','gravityformsenvoy'));
                else
                    $state_idarray = array('address_state',$state_fid);

                $zip_fid           = $envoyrechargemeta['envoyzip_' . $form_id];
                if(empty($zip_fid)) array_push($errors, __('Error: EnvoyRecharge zip or postcode feed field is empty.','gravityformsenvoy'));
                else
                    $zip_idarray = array('address_postcode',$zip_fid);

                $country_fid       = $envoyrechargemeta['envoycountry_' . $form_id];
                if(empty($country_fid)) array_push($errors, __('Error: EnvoyRecharge country feed field is empty.','gravityformsenvoy'));
                else
                    $country_idarray = array('address_country',$country_fid);

                $amount_id         = $envoyrechargemeta['envoyamount_' . $form_id];
                if(empty($amount_id)) array_push($errors, __('Error: EnvoyRecharge amount feed field is empty.','gravityformsenvoy'));
                else{
                    if($amount_id !='total')$amount_idarray = array('amount',$amount_id); // check that amount field is not a total field.
                }
                    

                $startdate_id         = $envoyrechargemeta['envoystartdate_' . $form_id];
                if(!empty($startdate_id)) $startdate_idarray = array('startdate',$startdate_id);
                $enddate_id         = $envoyrechargemeta['envoyenddate_' . $form_id];
                if(!empty($enddate_id)) $enddate_idarray = array('enddate',$enddate_id);
                $currency_id         = $envoyrechargemeta['envoycurrency_' . $form_id];
                if(!empty($currency_id)) $currency_idarray = array('currency',$currency_id);

                $paymentgateway_fid = $envoyrechargemeta['envoygateway_' . $form_id];
                if(empty($paymentgateway_fid)) array_push($errors, __('Error: EnvoyRecharge Gateway id feed field is empty.','gravityformsenvoy'));
        

                $txref_fid = $envoyrechargemeta['envoytxref_' . $form_id];
                if(!empty($txref_fid)) $txtref_idarray = array('txref',$txref_fid);
                $reference_fid = $envoyrechargemeta['envoyreference_' . $form_id];
                if(!empty($reference_fid)) $reference_idarray = array('reference',$reference_fid);
                ...
                if(sizeof($errors) > 0){
                    ...
                    foreach ($errors as $key => $feed_error) {
                        $settings_errors .= '<li >'."\n";
                        $settings_errors .= __($feed_error,'gravityformsenvoy')."\n";
                        $settings_errors .= '</li>'."\n";
                    }
                    $settings_errors .= '</ul>'."\n";
                    $settings_errors .= '</li>'."\n";
                ...
                if(!empty($address2_idarray)) array_push($fields_in_feed, $address2_idarray);
                ...
                if(isset($amount_idarray)) array_push($fields_in_feed, $amount_idarray);
                if(!empty($startdate_idarray)) array_push($fields_in_feed, $startdate_idarray);
                if(!empty($enddate_idarray)) array_push($fields_in_feed, $enddate_idarray);
                if(!empty($currency_idarray)) array_push($fields_in_feed, $currency_idarray);
                if(!empty($txtref_idarray)) array_push($fields_in_feed, $txtref_idarray);
                if(!empty($reference_idarray)) array_push($fields_in_feed, $reference_idarray);
                   ...
                   foreach ($form["fields"] as $fieldkey => $field) {
                        if($field['type']!='creditcard' && $field['type']!='interval' && $field['type']!='frequency'){
                            ...
                            if($field['id']==$feed_array[0]){
                                ...
                                if($field['type']=='product') $total += $this->clean_amount($amount)/100;
                            }
                        }
                            ...
                            if ($ccnumber == '' || $ccdate_month == '' || $ccdate_year == '' || $ccv == '' || $ccname == '') {
                                ...
                                $field["validation_message"]   = __('Error: Credit Card information not entered correctly.','gravityformsenvoy');
                ...
                if ($validation_result["is_valid"]) {
                    ...
                    $data["interval"]   = (isset($interval) && $interval !='' && $interval !=null) ? $interval : 'one-off';
                    $data["frequency"]  = (isset($frequency) && $frequency !='' && $frequency !=null) ? $frequency : 1;
                    if(isset($startdate)) $data["startdate"]  = $startdate ; //format need to be date('d/m/Y');
                    if(isset($enddate)) $data["enddate"]    = $enddate; // format need to be date('d/m/Y');
                    if(isset($txtref)) $data["txref"]      = $txtref;
                    ...
                    if(isset($address_line2)) $data["address_line2"]    = $address_line2;
                    ...
                    if (isset($result->subscription->status) || $result->transaction->status) {
                        return $validation_result;
                    } else {
                        $validation_result["is_valid"] = false;
                        $error_message = '<ul>'."\n";
                        ...
                        if($result == null || $result ==''){
                            $error_message .= '<li>'.__('Sorry Buddy. We could not connect to envoyrecharge at this time.Invalid credentials</li>','gravityformsenvoy')."\n";
                        }
                        else{
                            
                            $error_array = $result->_errors;
                            //$error_object = get_object_vars($result->_errors);
                            foreach ($error_array as $key => $error) {
                                $error_object = get_object_vars($error);

                                $error_message .= '<li>' . __($error_object['description'],'gravityformsenvoy') . '</li>'."\n";
                            }
                        }
                        $error_message .= '</ul>'."\n";
                        $error_message .= '</li>'."\n";
