<?php
add_filter('gform_add_field_buttons', 'envoy_add_field', 10, 1);

function envoy_add_field($field_groups) {
...
add_filter('gform_field_type_title', 'envoy_field_title', 5, 2);

function envoy_field_title($title, $field_type) {
    if ($field_type == 'envoyrecharge')
        return __('EnvoyRecharge', 'gravityformsenvoyrecharge');
    return $title;
}

// Adds the input area to the external side
add_action("gform_field_input", "envoy_field_field_input", 10, 5);

function envoy_field_field_input($input, $field, $value, $lead_id, $form_id) {
...
add_action("gform_editor_js", "envoy_field_gform_editor_js");

function envoy_field_gform_editor_js() {
?>
...
add_action("gform_field_standard_settings", "envoy_field_settings", 10, 2);

function envoy_field_settings($position, $form_id) {
...
add_action("gform_editor_js_set_default_values", "envoy_default_frequencies");

function envoy_default_frequencies() {
?>
        case 'envoyrecharge':
...
add_filter('gform_tooltips', 'envoy_add_field_tooltips');

function envoy_add_field_tooltips($tooltips) {
...
add_action('gform_enqueue_scripts', 'envoy_field_gform_enqueue_scripts', 10, 2);

function envoy_field_gform_enqueue_scripts($form, $ajax) {
...
add_action("gform_field_css_class", "envoy_custom_field_class", 10, 3);
...
add_filter("gform_entry_field_value", "envoy_field_entry_output", 10, 4);

function envoy_field_entry_output($value, $field, $lead, $form){
    if ($field["type"] == "envoyrecharge"){
...
add_filter('gform_field_validation', 'envoy_field_validation', 1, 4);

function envoy_field_validation($result, $value, $form, $field) {
    if ($field['type'] == 'envoyrecharge') {
//         var_dump($field, $value, $result);exit;
